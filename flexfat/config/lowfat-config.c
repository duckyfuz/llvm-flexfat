 ...
 *
 * FlexFat: this is the FlexFat config/table generator, a faithful port of the
 * reference LowFat config/lowfat-config.c. It is a standalone host tool (no LLVM
 * API surface), and its emitted lowfat_config.{c,h} / lowfat.ld must stay
 * BYTE-IDENTICAL to the reference, so the generation logic, the magic schemes
 * (POW2 ~(size-1); non-POW2 ceiling reciprocal floor(2^64/size)+1), the cached
 * precision-error computation, and the clzll-based lowfat_heap_select() dispatch
 * are preserved exactly. Build like the reference:
 *     cc -O2 lowfat-config.c -lm -lpthread
 */
