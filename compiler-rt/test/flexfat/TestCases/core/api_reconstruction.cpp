...
#define flexfat_base(p)        __flexfat_get_base((uptr)(p))
#define flexfat_size(p)        __flexfat_get_size((uptr)(p))
#define flexfat_offset(p)      __flexfat_get_offset((uptr)(p))
#define flexfat_usable_size(p) __flexfat_get_usable_size((uptr)(p))

int main() {
  size_t request_size = 100;
  char *p = (char *)malloc(request_size);
  uptr base = flexfat_base(p);
  uptr size = flexfat_size(p);

  printf("Base and Interior Pointers:\n");
  // Test start of object
  if (flexfat_base(p) == (uptr)p) printf("  start: ok\n");
  // Test interior pointer
  if (flexfat_base(p + 50) == (uptr)p) printf("  interior: ok\n");
  ...
  if (flexfat_offset(p) == 0) printf("  offset_0: ok\n");
  if (flexfat_offset(p + 50) == 50) printf("  offset_50: ok\n");
  if (flexfat_usable_size(p) == size) printf("  usable_start: ok\n");
  if (flexfat_usable_size(p + 50) == size - 50) printf("  usable_50: ok\n");

  printf("Non-Fat Pointer Handling:\n");
  // Standard stack pointer (not instrumented yet in this test)
  int x;
  if (flexfat_base(&x) == 0) printf("  stack_base: ok\n");
  if (flexfat_size(&x) == (uptr)-1) printf("  stack_size: ok\n");

  // NULL pointer
  if (flexfat_base(NULL) == 0) printf("  null_base: ok\n");
  if (flexfat_size(NULL) == (uptr)-1) printf("  null_size: ok\n");
