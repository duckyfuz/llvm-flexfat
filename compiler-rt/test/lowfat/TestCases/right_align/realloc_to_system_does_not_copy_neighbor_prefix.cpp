...
extern "C" std::uintptr_t __lf_get_base(std::uintptr_t ptr);
...
int main() {
  ...
  char *neighbor_slot_base =
      reinterpret_cast<char *>(__lf_get_base(reinterpret_cast<std::uintptr_t>(neighbor)));
  memset(neighbor_slot_base, 0x5A, 16);

  // Force the LowFat -> system-malloc migration path in realloc().
