 ...
 *
 * FlexFat: the runtime ABI header, a verbatim port of the reference LowFat
 * lowfat.h. The inline pointer-encoding accessors (lowfat_index/size/magic/
 * objidx/base/buffer_size) ARE the codegen contract — POW2 base is a single
 * `and`, non-POW2 base is a 128-bit reciprocal multiply (never a div) — so the
 * bodies are kept byte-identical to the reference. "FlexFat" is branding only.
 */
