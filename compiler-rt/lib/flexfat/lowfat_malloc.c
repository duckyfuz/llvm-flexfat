 ...
 *
 * FlexFat: the per-size-class bump+freelist heap allocator, a faithful port of
 * the reference LowFat lowfat_malloc.c. It is #included into lowfat.c as one
 * translation unit (after the encoding accessors and the mutex/rand/error/
 * dont_need helpers it depends on). The malloc/free fast paths (freelist LIFO
 * pop, bump pointer, lazy mprotect commit, per-region mutex) are kept verbatim
 * for performance parity. The libc interposition aliases are guarded by
